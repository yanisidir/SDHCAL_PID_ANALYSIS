...
def load_and_split_data(pion_path, proton_path, test_size=0.2, val_size=0.1):
    """ Load the data and slice them by group according to event_index"""

    # Adds label 0 and 1 for pion and proton, and adds event_id by merging event_index + _typeofparticle to get a unique event_id
    ...
    # Merge pion and proton dataframe + the index from the 2nd contact folows the last one, so 0,1,2 + 0,1,2 becomes 0,1,2,3,4,5 and not 0,1,2,0,1,2
    full_df = pd.concat([pion_df, proton_df]).reset_index(drop=True)
    
    # Apply a smearing of 100ps
    np.random.seed(config['seed_gaus'])
    full_df['time'] += np.random.normal(loc=0.0, scale=config['sigma_gaus'], size=full_df['time'].shape)

    # Truncate the value to 0.01 of precision ( bins of 10ps )
    full_df['time'] = (full_df['time'] * 100).astype(int) / 100

    # Get 2 arrays, one for the event_id, the other one for the label 
...
def build_graph(features, time, k=8, mode='route', time_window=2.0):
    ...
    edge_index = knn_z(edge_features, k, mode, device)
