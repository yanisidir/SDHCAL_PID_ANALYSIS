...
void EnergyReco() {
    ...
    TFile* fHits   = TFile::Open("/gridgroup/ilc/midir/analyse/data/digitized/130k_pi_E1to130_digitized.root");
    TFile* fEnergy = TFile::Open("/gridgroup/ilc/midir/analyse/data/raw/130k_pi_E1to130.root");
    if (!fHits || !fEnergy) {
    ...
    // Paramètre a0
    minuit->DefineParameter(0, parNames[0], /*start=*/0.0430851, /*step=*/1e-5, /*min=*/0.0, /*max=*/0.0);
    // Paramètre a1
    minuit->DefineParameter(1, parNames[1], /*start=*/-3.50665e-05, /*step=*/1e-5,  /*min=*/0.0, /*max=*/0.0);
    // Paramètre a2
    minuit->DefineParameter(2, parNames[2], /*start=*/1.94847e-08, /*step=*/1e-5,  /*min=*/0.0, /*max=*/0.0);

    // Paramètre b0
    minuit->DefineParameter(3, parNames[3],  /*start=*/0.107201, /*step=*/1e-5,  /*min=*/0.0, /*max=*/0.0);  
    // Paramètre b1
    minuit->DefineParameter(4, parNames[4],  /*start=*/-6.36402e-05, /*step=*/1e-5,  /*min=*/0.0, /*max=*/0.0);
    // Paramètre b2
    minuit->DefineParameter(5, parNames[5],  /*start=*/1.20235e-08, /*step=*/1e-5,  /*min=*/0.0, /*max=*/0.0);

    // Paramètre c0
    minuit->DefineParameter(6, parNames[6],  /*start=*/1.81865e-13, /*step=*/1e-5,   /*min=*/1e-10, /*max=*/0.0);  
    // Paramètre c1
    minuit->DefineParameter(7, parNames[7],  /*start=*/0.000735489, /*step=*/1e-5,  /*min=*/0.0, /*max=*/0.0);
    // Paramètre c2
    minuit->DefineParameter(8, parNames[8], /*start=*/-3.00925e-07, /*step=*/1e-5, /*min=*/0.0, /*max=*/0.0);  // 
    minuit->Migrad();
    ...
    leg->Draw();
    cComp->SaveAs("plots/Reco_vs_Beam_pi.pdf");
    ...
    c1->SaveAs("plots/Profil_linearite_pi.pdf");
    ...
    c2->SaveAs("plots/Linearity_pi.pdf");
    ...
    gRel->Draw("APL");
    cRes->SaveAs("plots/Resolution_relative_pi.pdf");
}
