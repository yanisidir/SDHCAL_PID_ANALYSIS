// EnergyReco.C - Reconstruction de l'énergie primaire du faisceau
...
void EnergyReco(const std::string& particle) {
    ...
    double depenergy = 0.0, primenergy = 0.0;
    ...
    for (Long64_t i = 0; i < nEntries; ++i) {
        ...
        if (primenergy > 0.0) {
            ...
            gEdep.push_back(depenergy); gEbeam.push_back(primenergy);
        }
    }
    ...
    leg->Draw();
    cComp->SaveAs(("plots/Reco_vs_Beam" + tag + ".pdf").c_str());
    ...
    c1->SaveAs(("plots/Profil_linearite" + tag + ".pdf").c_str());
    ...
    c2->SaveAs(("plots/Linearity" + tag + ".pdf").c_str());
    ...
    gRel->Draw("APL");
    cRes->SaveAs(("plots/Resolution_relative" + tag + ".pdf").c_str());
