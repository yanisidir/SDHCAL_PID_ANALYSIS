#!/usr/bin/env python3
# PID_Inference_with_Parametric_Energy_noPID_pi_p.py
# Même sortie que la version PID, mais sans classif : routage par l'identité vraie (PDG)
# Étude binaire π vs p (kaons exclus) + arguments hardcodés
...
class InferenceCfg:
    ...
    # ### CHANGEMENT: mappings binaire π/p uniquement
...
# ### CHANGEMENT: dictionnaire sans kaon
PAR_BY_TAG = {
    "pi": PAR_PI,
    "proton": PAR_PROTON,
}
...
def energy_parametric(N1: np.ndarray, N2: np.ndarray, N3: np.ndarray, P: np.ndarray) -> np.ndarray:
    """E_reco = (a0+a1*N+a2*N^2)*N1 + (b0+b1*N+b2*N^2)*N2 + (g0+g1*N+g2*N^2)*N3,
    avec N = N1+N2+N3. Vectorisé."""
...
def run_inference(files, out_csv):
    ...
    # --- Colonnes de sortie (compatibles, mais sans kaon) ---
...
def main():
    # === chemins hardcodés ===
    files = [
        "/gridgroup/ilc/midir/analyse/data/validation_set_pion_proton.root"
    ]

    # out_csv = "no_pid_chi2.csv"  

    out_csv = "no_pid_chi2_pion_all.csv"  
