...
# - plots/Resolution_relative_all_LGBM.png
# - plots/Lin_profile_all_LGBM.png
# - plots/Relative_deviation_all_LGBM.png
# - plots/Lin_n_Dev_all_LGBM.png
...
INT_TO_CANON = {0: "pi", 1: "proton", 2: "kaon"}
TEXT_TO_CANON = {
    "pi": "pi", "π-": "pi", "pi-": "pi", "pion": "pi",
    "p": "proton", "proton": "proton",
    "k": "kaon", "k0": "kaon", "k⁰": "kaon", "k0s": "kaon", "kaon": "kaon",
}

# Binning fixe : 10 -> 100 GeV, 20 bins
E_MIN_FIXED = 10.0
E_MAX_FIXED = 100.0
NBINS_FIXED = 10

# ΔE histo/fit (comme la macro)
DELTAE_NBINS = 100
DELTAE_MIN = -20.0
DELTAE_MAX =  20.0
...
def _bin_edges_fixed():
    return np.arange(5.0, 105.0 + 1e-9, 10.0) 
...
def load_csv_grouped(csv_path: Path) -> Dict[str, Tuple[np.ndarray, np.ndarray]]:
    """
    Lit le CSV et renvoie un dict { 'pi'|'kaon'|'proton' : (y_true, y_pred) }.
    Colonnes requises : E_true_GeV, E_pred_GeV.
    Particule déduite de label_true_int (priorité) puis label_true (texte).
    """
    ...
    df["E_true"] = df["E_true_GeV"].astype(float)
    df["E_reco"] = df["E_pred_GeV"].astype(float)
    ...
    for label in ("pi", "kaon", "proton"):
        sub = df[df["particle"] == label]
        if not sub.empty:
            out[label] = (sub["E_true"].to_numpy(float),
...
def plot_linearity_multi(results: Mapping[str, Tuple[np.ndarray, np.ndarray]], out_png: Path):
    ...
    frame.SetMinimum(0.0); frame.SetMaximum(100.0)

    c = TCanvas("cLinAll", "Linearity profile (pi,K,p) ", 900, 650)
    frame.Draw(); keep += [frame, c]

    diag = TLine(0.0, 0.0, 100.0, 100.0)
    diag.SetLineColor(ROOT.kGray+2); diag.SetLineStyle(2); diag.Draw("SAME")
    keep.append(diag)
    ...
    leg.Draw(); c.Update(); c.SaveAs(str(out_png))
...
def plot_deviation_multi(results: Mapping[str, Tuple[np.ndarray, np.ndarray]], out_png: Path):
    ...
    frame.SetMinimum(-10.0); frame.SetMaximum(10.0)

    c = TCanvas("cDevAll", "Relative deviation (%) (pi,K,p) ", 900, 650)
    frame.Draw(); keep += [frame, c]

    band = TBox(0.0, -1.0, 100.0, 1.0)
    band.SetFillColorAlpha(ROOT.kGray, 0.2); band.SetLineColor(0); band.Draw("SAME")
    keep.append(band)

    zero = TLine(0.0, 0.0, 100.0, 0.0)
    zero.SetLineColor(ROOT.kGray+2); zero.SetLineStyle(2); zero.Draw("SAME")
    keep.append(zero)
...
def plot_resolution_multi(results: Mapping[str, Tuple[np.ndarray, np.ndarray]], out_png: Path):
    ...
    frame.SetMinimum(0.0); frame.SetMaximum(20.0)
    ...
    for ln in (l5, l10):
        ln.SetLineColor(ROOT.kBlack); ln.SetLineStyle(2); ln.Draw("SAME")
    keep += [l5, l10]
...
def plot_linearity_and_deviation_combo(results: Mapping[str, Tuple[np.ndarray, np.ndarray]], out_png: Path):
    ...
    pad2.SetTopMargin(0.0); pad2.SetBottomMargin(0.25)
    pad1.Draw(); pad2.Draw()
    keep += [pad1, pad2]
    ...
    fr1.SetMinimum(0.0); fr1.SetMaximum(100.0)
    fr1.Draw(); keep.append(fr1)
    ...
    # Bas : déviation
    pad2.cd(); pad2.SetGridy(True)

    fr2 = TH1F("fr2", ";E_{true} [GeV];Biais relatif [%]", 100, 0.0, 100.0)
    fr2.SetMinimum(-11.0); fr2.SetMaximum(11.0)
    ...
    fr2.Draw(); keep.append(fr2)

    band = TBox(0.0, -1.0, 100.0, 1.0)
    band.SetFillColorAlpha(ROOT.kGray, 0.15); band.SetLineColor(0); band.Draw("SAME")
    keep.append(band)
    ...
    c.Update(); c.SaveAs(str(out_png))

# ------------------------------ Main ------------------------------
if __name__ == "__main__":
    # Chemin vers ton CSV

    # CSV_PATH = Path("pid_energy_LGBM.csv" )

    # CSV_PATH = Path("no_pid_energy_LGBM.csv" )

    # CSV_PATH = Path("pid_energy_param_LGBM.csv")

    # CSV_PATH = Path("no_pid_energy_param.csv")  

    CSV_PATH = Path("no_pid_energy_param_pion_to_all.csv")  

    OUT_DIR = Path("plots"); OUT_DIR.mkdir(parents=True, exist_ok=True)

    # 1) Charger et regrouper par espèce
    results = load_csv_grouped(CSV_PATH)
    # results: {"pi": (y_true, y_pred), "kaon": (...), "proton": (...)}

    # 2) Tracer (mêmes noms que la macro .npz)
    # plot_resolution_multi(results, OUT_DIR / "PID_Resolution_relative_LGBM.png")
    # plot_linearity_and_deviation_combo(results, OUT_DIR / "PID_Lin_n_Dev_LGBM.png")
    # plot_linearity_multi(results, OUT_DIR / "PID_Lin_profile_LGBM.png")
    # plot_deviation_multi(results, OUT_DIR / "PID_Relative_deviation_LGBM.png")

    # plot_resolution_multi(results, OUT_DIR / "no_PID_Resolution_relative_LGBM.png")
    # plot_linearity_and_deviation_combo(results, OUT_DIR / "no_PID_Lin_n_Dev_LGBM.png")
    # plot_linearity_multi(results, OUT_DIR / "no_PID_Lin_profile_LGBM.png")
    # plot_deviation_multi(results, OUT_DIR / "no_PID_Relative_deviation_LGBM.png")

    # plot_resolution_multi(results, OUT_DIR / "PID_Resolution_relative_param.png")
    # plot_linearity_and_deviation_combo(results, OUT_DIR / "PID_Lin_n_Dev_param.png")
    # plot_linearity_multi(results, OUT_DIR / "PID_Lin_profile_param.png")
    # plot_deviation_multi(results, OUT_DIR / "PID_Relative_deviation_param.png")

    # plot_resolution_multi(results, OUT_DIR / "no_PID_Resolution_relative_param.png")
    # plot_linearity_and_deviation_combo(results, OUT_DIR / "no_PID_Lin_n_Dev_param.png")
    # plot_linearity_multi(results, OUT_DIR / "no_PID_Lin_profile_param.png")
    # plot_deviation_multi(results, OUT_DIR / "no_PID_Relative_deviation_param.png")


    plot_resolution_multi(results, OUT_DIR / "PID_Resolution_relative_param_pion.png")
    plot_linearity_and_deviation_combo(results, OUT_DIR / "PID_Lin_n_Dev_param_pion.png")
    # plot_linearity_multi(results, OUT_DIR / "PID_Lin_profile_param_pion.png")
    # plot_deviation_multi(results, OUT_DIR / "PID_Relative_deviation_param_pion.png")
