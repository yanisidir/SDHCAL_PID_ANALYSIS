...
def heartbeat(msg="GridSearch still running...", every=30):
    # petit “ping” périodique dans les logs pour prouver que ça vit
    def _beat():
        tqdm.write(f"[HB] {msg} @ {time.strftime('%H:%M:%S')}")
        hb.timer = threading.Timer(every, _beat)
        hb.timer.daemon = True
        hb.timer.start()
    hb = type("HB", (), {})()
    _beat()
    return hb
...
class Config:
    ...
    file_path: str = (
        ...
        "/gridgroup/ilc/midir/analyse/data/merged_primaryEnergy/130k_kaon_E1to130_params_merged.root"
    )
    ...
    learning_rate: float     = 0.05     # 0.05, 0.03, 0.05
    n_estimators: int        = 2000     # 2000, 2500, 4000, 
    num_leaves: int          = 127       # 127, 255, 63,
    max_depth: int           = -1
    reg_alpha: float         = 0.0      # 0.0, 0.10
    reg_lambda: float        = 0.10     # 0.1, 0.30,0.40, 0.8,
    min_child_samples: int   = 20       # 20, 30, 40, 60
    subsample: float         = 0.8      # 0.8, 0.9, ,
    colsample_bytree: float  = 0.8      # 0.8, 0.9,0.7 ,
    min_gain_to_split: float = 0.8         
    verbosity: int           = -1      

    # Early stopping rounds
    early_stopping_rounds: int = 50

    # Grid‑search (set to True to enable)
...
PARAMETERS_FILE = Path("run_parameters_regression_kaon.csv")
COMMENTS_FILE = Path("run_comments_regression_kaon.csv")
PERF_FILE = Path("hadron_energy_performances_kaon.csv")
...
def init_run(config: Config) -> tuple[int, Dict[str, Path]]:
    ...
    paths = {
        "model": RES_DIR / "models" / f"lgbm_regressor_kaon_{run_id}.joblib",
        "scaler": RES_DIR / "models" / f"scaler_kaon_{run_id}.joblib",
...
def _check_zero_variance(X: np.ndarray, cols: Sequence[str]):
    std = X.std(axis=0)
    bad = [c for c, s in zip(cols, std) if s == 0]
    if bad:
        logger.warning("Colonnes à variance nulle détectées (supprime-les) : %s", bad)
...
def rel_rmse_logspace(y_true_log: np.ndarray, y_pred_log: np.ndarray):
    # labels et prédictions sont en log(E)
    y = np.exp(y_true_log)
    yhat = np.exp(y_pred_log)
    r = (yhat - y) / (y + 1e-12)
    # (name, value, is_higher_better). is_higher_better=False => plus petit est meilleur
...
def train_model(
    ...
    if not cfg.grid_search:
        ...
        # Progress bar
        n_candidates = len(cfg.param_grid["num_leaves"]) \
                     * len(cfg.param_grid["max_depth"]) \
                     * len(cfg.param_grid["learning_rate"]) \
                     * len(cfg.param_grid["n_estimators"])
        n_total = n_candidates * cv.get_n_splits()


        bar = tqdm(total=n_total, desc="GridSearch", mininterval=1, miniters=1, dynamic_ncols=True)
        hb = heartbeat(every=20)
        with tqdm_joblib(bar):
            grid.fit(X_tr, y_tr, callbacks=[lgb.log_evaluation(period=0)])
        hb.timer.cancel()
        bar.close()
        ...
        # Re-train final model with best params and full n_estimators
        reg = build_lgbm(cfg)
        reg.set_params(**best_params, n_estimators=cfg.n_estimators)
        reg.fit(
            X_tr,
            y_tr,
...
def evaluate(reg: lgb.LGBMRegressor, X_test: np.ndarray, y_test: np.ndarray):
    # Le modèle prédit logE
    y_pred_log = reg.predict(X_test)
    y_pred = np.exp(y_pred_log)      # <-- repasse en E
...
def run(cfg: Config) -> None:
    ...
    _check_zero_variance(X_tr, cfg.feature_cols)
