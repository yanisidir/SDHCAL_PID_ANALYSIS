...
void compareThr3() {
    ...
    // 2) Min/Max de Thr3
    Double_t minP  = tPion->GetMinimum("Thr3");
    Double_t maxP  = tPion->GetMaximum("Thr3");
    Double_t minPr = tProton->GetMinimum("Thr3");
    Double_t maxPr = tProton->GetMaximum("Thr3");

    std::cout << "Pion Thr3 range:   [" << minP  << ", " << maxP  << "]" << std::endl;
    std::cout << "Proton Thr3 range: [" << minPr << ", " << maxPr << "]" << std::endl;
    ...
    // 4) Histos
    TH1D* hPion   = new TH1D("hPionThr3",   "Thr3;Thr3;Evts", nbins, xmin, xmax);
    TH1D* hProton = new TH1D("hProtonThr3", ";Thr3;Evts",                   nbins, xmin, xmax);

    // 5) Remplissage
    Float_t dThr3;

    tPion->SetBranchStatus("*", 0);       // tout désactiver
    tPion->SetBranchStatus("Thr3", 1);    // activer Thr3
    tPion->SetBranchAddress("Thr3", &dThr3);
    ...
    tProton->SetBranchStatus("Thr3", 1);
    tProton->SetBranchAddress("Thr3", &dThr3);
    ...
    TCanvas* c = new TCanvas("cThr3", "Comparaison Thr3", 600, 600);
    c->SetGrid();
