...
void compareDensity() {
    ...
    // 2) Min/Max de Density
    Double_t minP  = tPion->GetMinimum("Density");
    Double_t maxP  = tPion->GetMaximum("Density");
    Double_t minPr = tProton->GetMinimum("Density");
    Double_t maxPr = tProton->GetMaximum("Density");

    std::cout << "Pion Density range:   [" << minP  << ", " << maxP  << "]" << std::endl;
    std::cout << "Proton Density range: [" << minPr << ", " << maxPr << "]" << std::endl;
    ...
    // 4) Histos
    TH1D* hPion   = new TH1D("hPionDensity",   "Density;Density;Evts", nbins, xmin, xmax);
    TH1D* hProton = new TH1D("hProtonDensity", ";Density;Evts",                   nbins, xmin, xmax);

    // 5) Remplissage
    Float_t dDensity;

    tPion->SetBranchStatus("*", 0);       // tout désactiver
    tPion->SetBranchStatus("Density", 1);    // activer Density
    tPion->SetBranchAddress("Density", &dDensity);
    ...
    tProton->SetBranchStatus("Density", 1);
    tProton->SetBranchAddress("Density", &dDensity);
    for (Long64_t i = 0; i < tProton->GetEntries(); ++i) {
        tProton->GetEntry(i);
        hProton->Fill(dDensity);
    }
    ...
    TCanvas* c = new TCanvas("cDensity", "Comparaison Density", 600, 600);
    c->SetGrid();
